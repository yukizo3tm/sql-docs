...
HA replicas use the same server and database name as the primary replica. Their Service Level Objective (SLO) also always the same as for the primary replica. HA replicas are not visible or manageable as stand-alone resources, from the portal or from any API. They are billed at the same compute rate as the primary replica, but storage costs do not apply to secondary replicas.
